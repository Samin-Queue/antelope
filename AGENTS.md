...
**레인을 중첩하지 않는다.** 레인은 게이트웨이(`runObject`·`runText`)와 브라우저
진입점(`renderPdf`·`probeCaptcha`·`runPlaywrightAgent`)이 건다. 호출부가
`Promise.all` 로 나란히 돌릴 때 `lanes.batch(() => writeDocument(…))` 처럼 **한 번
더 감싸면 안 된다** — 안쪽 `runText` 가 같은 레인을 또 기다리므로, 바깥 `limit` 개가
차는 순간 자기 자신을 기다리는 데드락이 된다. 단계 상한도 못 구한다(`withTimeout`
은 race 라 진 쪽이 슬롯을 쥔 채 남는다). 한 번 걸리면 **프로세스가 죽을 때까지**
그 레인을 쓰는 모든 실행이 멈춘다. 실측: 서류 「작성 2」에서 15분 무응답,
`/api/health` 는 `batch: inFlight 2 · waiting 2` 에 모델 호출 0 — 이 게이지가
진단의 전부였다. 재진입 면제로 풀지 않는다. 상한이 거짓이 되고 게이지가 안 잡으며,
`AsyncLocalStorage` 가 끊기는 경계에서 간헐적 데드락으로 되돌아온다.
