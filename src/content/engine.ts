...
export const engine = {
  ...
  journey: {
    ...
    synth: {
      ...
      body: "예전에는 파일을 준 입력과 링크만 준 입력이 다른 성능을 냈습니다. 파일이 없으면 Studio 를 한 번도 안 타고 Solar 가 요약에서 필드를 추측했습니다 — 분류 분기도, 준비 문서도, 원문 좌표도 없이. 그런데 그때도 읽을 내용은 이미 있었습니다. Document Parse 가 평문을 안 받을 뿐이었습니다.",
  ...
  runtime: {
    ...
    evals: {
      ...
      body: "순수 함수 골든셋 47개가 1초 안에 돕니다 — 의미 검증 16개, 재시도 판정 12개, 항목 정리 10개, 근거 매칭 8개. CI 에는 붙이지 않습니다.",
    },
  },
  ...
  relay: {
    ...
    sub: "같은 파이프라인을 다른 입구로 엽니다. 새 파이프라인을 만들지 않았습니다 — `runStart` 를 부르는 자리가 저장소 전체에 하나뿐이고, 슬랙 호스트가 그 한 줄을 씁니다.",
