 ...
 *
 * ⚠ **레인을 중첩하지 않는다.** `lanes.batch(() => … runText({ lane: "batch" }) …)`
 * 처럼 바깥이 슬롯을 쥔 채 안쪽이 같은 레인을 또 기다리면, 바깥 `limit` 개가
 * 차는 순간 안쪽은 영영 자리를 못 받는다 — 자기 자신을 기다리는 데드락이고,
 * 단계 상한도 못 구한다(`withTimeout` 은 race 라 진 쪽이 슬롯을 쥔 채 남는다).
 * 한 번 걸리면 프로세스가 죽을 때까지 그 레인을 쓰는 모든 실행이 멈춘다.
 * 레인은 **게이트웨이(`runObject`·`runText`)와 브라우저 진입점**이 건다.
 * 호출부가 `Promise.all` 로 나란히 돌릴 때는 감싸지 않는다 — 안쪽이 이미 잡는다.
 * (실측: 서류 작성 2편에서 batch 레인이 잠겨 15분 무응답, `/api/health` 는
 * `inFlight 2 · waiting 2` 에 모델 호출 0.)
 */
