...
한 화면에서 이어지는 플로우. 기존 lab 셋(samson · michael · notice)을 부품으로 쓴다.
...
| 2   | 요약 + good/bad 판정   | Samson(Studio) · `solar-mini`      | Markdown 요약, 판정                 |
| 3   | 추가 조사(크롤링)      | fetch + `solar-pro4`               | 첨부 파일 추가, 신청 URL, 입력 항목 |
| 4   | 정밀 분석              | Michael(Studio)                    | 분류·검증된 필드 JSON → 입력 항목   |
...
src/app/(app)/app/start/
  ...
  _lib/intake.ts       1단계
  _lib/summarize.ts    2단계 (Samson → 없으면 Solar 직접 요약) + 판정
  _lib/research.ts     3단계
  _lib/analyze.ts      4단계 (Michael → 없으면 Solar 직접 도출)
  _lib/prefill.ts      5단계
...
`Need` 병합 규칙: Michael 결과가 우선, research 가 보충. `key` 는 공백·기호를 뺀 라벨이다.
...
**2 summarize.** 파일마다 Samson(`UPSTAGE_SAMSON_AGENT_ID`) 을 돌려 Markdown 을 받는다.
...
**4 analyze.** 파일 전부를 Michael(`UPSTAGE_MICHAEL_AGENT_ID`) 한 job 에 넣는다.
`fields[]` 를 zod 로 느슨하게 받아 `Need[]`(source: michael) 로 바꾼다. 파일이 없거나
...
- 단계 하나의 실패는 `stage:error` 로 알리고 가능한 한 다음 단계로 간다
  (예: Michael 실패 → research Need 만으로 진행).
...
- `/demo/hiring` 링크 입력으로 1→9 를 실제로 돌린다 (Samson·Michael 은 Studio 실호출).
