...
src/app/(labs)/lab/relay/
  ...
  _lib/
    ...
    queue.ts      동시 실행 상한 (전역 2 · 사용자당 1)
    answers.ts    자유 텍스트 → needs 배분
    files.ts      첨부 → 공고 입력 / 제출 서류 + 보관함
    oidc.ts       Sign in with Slack — 계정 연결 동의 화면

src/app/(app)/app/settings/
  relay/route.ts             연결 시작(동의 화면으로 302)·해제
  relay/callback/route.ts    동의 후 돌아오는 자리
...
**연결은 동의 화면 한 번이다.** — 구현하며 바뀐 부분이다.

초안은 일회용 코드를 DM 으로 옮기게 했다. 실제로 붙여 보니 그건 우회책이고,
이 저장소에는 이미 옳은 패턴이 있다 — 구글 캘린더 연동(`linkSocial` + `/app/settings`).
슬랙에는 Sign in with Slack(OpenID Connect)이 있다.

```
/app/settings → 「슬랙 연결」
  → /app/settings/relay 가 서명된 state 와 함께 슬랙 동의 화면으로 302
  → 사용자가 어느 슬랙 계정인지 그 화면에서 직접 고른다
  → /app/settings/relay/callback 이 code 를 userInfo 로 바꿔 relay_identities 에 쓴다
```

- **콜백은 세션이 아니라 `state` 를 믿는다.** 동의 화면을 거치는 동안 다른
  계정으로 갈아탔을 수 있고, 그때 「지금 로그인한 사람」에게 슬랙 id 를 붙이면
  남의 계정이 연결된다. state 는 HMAC 으로 묶고 10분에 만료시킨다.
- state 를 서버에 저장하지 않는다 — 인스턴스가 재시작해도 진행 중인 동의가 살아야 한다.
- `redirect_uri` 는 요청에서 추측하지 않는다. 슬랙에 등록한 값과 글자 하나라도
  다르면 `bad_redirect_uri` 이고, 프록시 뒤에서는 `req.url` 이 내부 주소다.
- 중간에 시도했다 **버린 것**: 슬랙 프로필 이메일이 같으면 자동 연결. 스코프
  하나로 되지만 사용자가 동의한 적 없는 연결이 생긴다 — 동의 화면이 그 자리를 대신한다.
...
### Step 1 — 호스트와 중계 ✅ (검증 21항목)

`host.ts` 가 `runStart` 를 부르고, `sink.ts` 가 이벤트를 스레드로 옮기고,
`queue.ts` 가 동시 실행을 막는다.

- **첫 표시줄만 스로틀을 타지 않는다.** 3초를 기다리게 하면 표시줄이 실행이 끝날 때 한 번 생기고 만다 — 실측으로 잡았다. 그 분기가 없으면 카드 댓글이 표시줄보다 먼저 뜬다.
- 발신을 한 줄로 세운다. 동시에 때리면 댓글 순서가 뒤바뀌고 한도에도 먼저 닿는다.
- 큐는 **자리를 비웠다 채우지 않고 다음 사람에게 그대로 넘긴다** — 비우면 그 틈에 새 요청이 상한을 통과한다.

### Step 2 — 되묻기 ✅ (검증 11항목)

빈 항목이 있으면 스레드가 `asking` 으로 열린 채 남고, 답장 한 통을 항목들에 나눠 담는다.

- **항목을 특정할 근거가 글에 있어야 채운다.** 프롬프트로는 막히지 않았다 —
  실측: 「음 그건 좀 나중에 알려줄게요. 아 그리고 1999-04-12」에서 모델이 그
  날짜를 법인 설립일로 단정했고, 규칙을 몇 줄 더 붙여도 계속 그랬다. 그래서
  라벨의 낱말 중 하나가 사람이 쓴 글에 실제로 있어야 채우게 했다. 남은 항목이
  하나뿐이면 건너뛴다.
- 대가는 「우리 회사는 삼인큐예요」가 버려지는 것이다. 사용자 명의로 접수되는
  값이라 조용히 틀리는 쪽이 더 비싸다.
- 받은 값은 `goals.snapshot` 과 `memories` 양쪽에 남는다.

### Step 3 — 파일 ✅ (검증 13항목)

공고 파일을 붙여 멘션하면 그것으로 준비가 돌고, 되묻는 중 올린 서류는 이번
신청(`artifactDir`)과 보관함(`user_documents`) 양쪽에 남는다.

- **파일이 글보다 먼저다.** 「사업자등록증이야」는 글이 아니라 첨부의 라벨이라,
  항목 배분에 그대로 넣으면 엉뚱한 칸을 채운다.
- ⚠ `documentKey("제출 서류") === ""` — NOISE 가 두 낱말을 다 지운다. 빈 키를
  매칭에 쓰면 `includes` 가 항상 참이라 **아무 파일이나 그 항목에 붙는다**.
  개편 문서가 `pick()` 에서 지적한 결함의 다른 얼굴이고, 여기서는 빈 키를 후보에서 뺐다.
- 받을 서류가 둘 이상인데 파일명·문맥 어느 쪽으로도 못 고르면 짐작하지 않고 되묻는다.
