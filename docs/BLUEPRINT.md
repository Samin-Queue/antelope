...
| 축              | 책임                                                                                      | Agent ID (환경변수)           |
| --------------- | ----------------------------------------------------------------------------------------- | ----------------------------- |
| **유효성 검사** | **파일 유효성 검사.** 받은 파일이 요청을 수행할 만한 정보를 담고 있는가를 가볍게 판정한다 | `UPSTAGE_VALIDATION_AGENT_ID` |
| **정보 분석**   | **파일 분석·필드 추출·정규화.** 정돈된 필드 목록과 최초 parse 된 Markdown 을 함께 낸다    | `UPSTAGE_ANALYSIS_AGENT_ID`   |
...
7. ~~파일 에이전트~~ — **완료(PDF 만).** `_lib/file-agent.ts`. 제출 서류를
   ...
   **발급 서류는 보관함으로 관리한다** (`_lib/documents.ts`, `user_documents` 테이블).
   사업자등록증·4대보험 명부는 공고마다 같은 것을 낸다 — 한 번 올리면 다음
   공고에서 자동으로 꺼내 쓴다. 조회는 이름에서 「사본·1부·서류」 같은 접미어를
   털어 맞춘다: `사업자등록증 사본` = `사업자등록증` = `사업자등록증 사본 1부`.
   같은 서류를 다시 올리면 갈아끼운다 — 새로 발급받은 쪽이 항상 맞다.

   **포맷 5종을 낸다** — pdf · hwp · hwpx · docx · xlsx. 공고가 요구한 형식을
   모델이 판단한다(「HWP 양식」→ hwp, 「엑셀 내역서」→ xlsx). 본문은 Markdown 으로
   한 번만 쓰고 `render/` 의 렌더러가 각자 옮긴다 — 포맷마다 프롬프트를 따로 두면
   같은 문서가 포맷별로 다른 내용이 된다.

   hwp 는 `@rhwp/core`(Rust+WASM, MIT). 번들링이 안 돼 별도 프로세스로 돌린다 —
   AGENTS.md 참고. 실패하면 PDF 로 떨어진다.

   **지정 서식도 채운다.** 첨부에 hwp·hwpx 가 있으면 새로 쓰지 않고 그것을
   연다 — 기관은 제 서식으로 받기를 원하고, 새로 만든 문서는 접수처에서
   반려된다. 표의 라벨 셀을 찾아 같은 행의 **빈** 칸에 값을 넣는다. 이미 적힌
   칸은 건드리지 않는다. 채운 칸이 하나도 없으면 서식이 아니었던 것이라 버린다.

   ⚠ 남은 것: docx·xlsx **서식 채우기**. xlsx 는 `exceljs` 가 읽기·쓰기를 다
   하므로 길이 있고, docx 는 라이브러리가 마땅치 않다.
